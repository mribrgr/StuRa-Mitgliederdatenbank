...
def list(request):
    ...
    if searchterm is not None:
        ...
        for term in searchterms:
            # Fetch all entries matching the search term
            ...
            mitgliederAemter = mitgliederAemter | MitgliedAmt.history.filter(Q(mitglied__id__icontains=term) | Q(mitglied__vorname__icontains=term) | Q(mitglied__name__icontains=term) | Q(amt__id__icontains=term) | Q(amt__bezeichnung__icontains=term) | Q(amt__referat__bezeichnung__icontains=term) | Q(amt__unterbereich__bezeichnung__icontains=term))
            ...
            aemter = aemter | Amt.history.filter(Q(id__icontains=term) | Q(bezeichnung__icontains=term) | Q(referat__id__icontains=term) | Q(referat__bezeichnung__icontains=term) | Q(unterbereich__id__icontains=term) | Q(unterbereich__bezeichnung__icontains=term))
            rechte = rechte | Recht.history.filter(Q(id__icontains=term) | Q(bezeichnung__icontains=term))
            aemterRechte = aemterRechte | AmtRecht.history.filter(Q(amt__id__icontains=term), Q(amt__bezeichnung__icontains=term) | Q(recht__id__icontains=term) |Q(recht__bezeichnung__icontains=term))
